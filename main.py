...
def get_ip_info(ip, bearer_token):
    ...
    # response = requests.post(url, headers=headers, data=json.dumps([ip.strip()]))
...
def process_output_file(output_file, output_file_csv):
    ...
    Input: output_file - path to the output text file, json format
    Output: output_file_csv - path to the output CSV file
    """
    data_params = [
        'ip',
        # 'hostname',
